...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        if self.loss_function is LossFunction.MSE:
            ...
            return -1 / len(y) * x.T @ np.sign(y - self.predict(x))
        elif self.loss_function is LossFunction.Huber:
            delta = 1
            z = np.where(np.abs(y - self.predict(x)) <= delta, y - self.predict(x), delta * np.sign(y - self.predict(x)))
            return  -1 / len(y) * x.T @ z
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        indexes = np.random.randint(0, len(y), size=self.batch_size)
        x_batch = x[indexes]
        y_batch = np.array(y)[indexes]  
        if self.loss_function is LossFunction.MSE:
            return -2 / len(y_batch) * x_batch.T @ (y_batch - self.predict(x_batch))
        elif self.loss_function is LossFunction.LogCosh:
            return 1 / len(y_batch) * x_batch.T @ np.tanh(self.predict(x_batch) - y_batch)
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        diff = self.lr() / (np.sqrt(v_est) + self.eps) * m_est
        self.w -= diff
        return -diff
...
class AdaMax(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        diff = self.lr() / (self.u + self.eps) * m_est
        self.w -= diff
        return -diff
